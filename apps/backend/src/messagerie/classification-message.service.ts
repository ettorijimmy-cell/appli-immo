...
import { candidat, contact, locataires, type Database } from "db";
 ...
 * scopée à l'organisation courante — la décision (une correspondance ->
 * classée, zéro ou plusieurs -> non_classe) est déléguée à
 * resoudreClassificationEmail (packages/core, fonction pure). Utilisé à la
 * fois par ImapSyncJobService (messages reçus) et SmtpEnvoiService
...
export class ClassificationMessageService {
  ...
  async resoudre(email: string, organisationId: string): Promise<ResultatClassification> {
    const [contactsCorrespondants, locatairesCorrespondants, candidatsCorrespondants] = await Promise.all([
      this.db
