...
import { createSci, archiveSci, listScis, type CreateSciInput, type Sci } from "../scis/api";
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-lg font-semibold">Patrimoine — SCI</h1>
        <button
          type="button"
          ...
          <tbody>
            {scis.map((sci) => (
              <tr key={sci.id} className="border-b border-slate-100">
                <td className="py-2">
                  <Link to={`/patrimoine/scis/${sci.id}`} className="text-indigo-700 hover:underline">
                    {sci.nom}
                  </Link>
                </td>
